...
class TargetAgent(SinglePurposeAgent):
    ...
    def process_bundles(self, input_bundles: Dict[str, Bundle]) -> Dict[str, Bundle]:
        ...
        target_preferences = exp_data.get_asset("target_preferences", {})
        ...
        if "likelihood_parameters" in result:
            target_bundle.add_asset("likelihood_parameters", result["likelihood_parameters"])
        
        if "target_gradients_wrt_sf" in result:
            target_bundle.add_asset("target_gradients_wrt_sf", result["target_gradients_wrt_sf"])
            
        return {"target_data": target_bundle}
    
    def _calculate_ml_target(self, f_model, f_obs, exp_data):
        """
        ...
        # Common reflections
        f_obs_common, f_model_common = f_obs.common_sets(f_model, assert_is_similar_symmetry=False)
        ...
        # Calculate target
        target_result = target_functor.target_and_gradients(
            f_model=f_model_common
        )
    ...
    def _calculate_ls_target(self, f_model, f_obs, exp_data):
        """
    ...
    def _calculate_lsf_target(self, f_model, f_obs, exp_data):
        """
    ...
    def _calculate_r_factor(self, f_obs, f_model):
        """
    ...
    def get_computation_info(self) -> Dict:
        ...
        return {
            ...
            "supported_targets": ["maximum_likelihood", "least_squares", "least_squares_f"],
