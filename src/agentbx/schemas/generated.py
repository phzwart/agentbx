...
class TargetDataBundle(BaseModel):
    ...
    # Optional assets
    r_factors: Dict[str, Any] = Field(default=None, description="Crystallographic R-factors")
    target_per_reflection: Any = Field(default=None, description="Target contribution from each reflection")
    likelihood_parameters: Dict[str, Any] = Field(default=None, description="Maximum likelihood alpha and beta parameters")
...
class GradientDataBundle(BaseModel):
    ...
    # Required assets
    coordinate_gradients: Any = Field(description="Gradients w.r.t. atomic coordinates: dT/d(xyz)")

    # Optional assets
    bfactor_gradients: Any = Field(default=None, description="Gradients w.r.t. B-factors: dT/d(B)")
    occupancy_gradients: Any = Field(default=None, description="Gradients w.r.t. occupancies: dT/d(occ)")
    ...
    def validate_dependencies(self, available_bundles: Dict[str, 'BaseModel']) -> bool:
        """Validate that all dependencies are satisfied."""
        required_deps: List[str] = ['xray_atomic_model_data', 'target_data', 'structure_factor_data'] or []
        for dep in required_deps:
            if dep not in available_bundles:
                raise ValueError(f'Missing dependency: {dep}')
        return True
...
class GeometryGradientDataBundle(BaseModel):
    ...
    geometric_gradients: Any = Field(description="Gradients of geometry restraints w.r.t. coordinates")

    # Optional assets
    restraint_energies: Dict[str, Any] = Field(default=None, description="Individual restraint energies by type")
    restraint_counts: Dict[str, Any] = Field(default=None, description="Number of restraints by type")
    geometry_metadata: Dict[str, Any] = Field(default=None, description="Metadata about geometry restraint computation")
...
class AgentConfigurationBundle(BaseModel):
    ...
    # Optional assets
    security_policies: Dict[str, Any] = Field(default=None, description="Security policies for the agent")
    resource_limits: Dict[str, Any] = Field(default=None, description="Resource limits for the agent")
    monitoring_config: Dict[str, Any] = Field(default=None, description="Monitoring configuration for the agent")
...
class MacromoleculeDataBundle(BaseModel):
    ...
    # Required assets
    pdb_hierarchy: Any = Field(description="CCTBX PDB hierarchy with full atomic model information")
    crystal_symmetry: Any = Field(description="Crystal symmetry (unit cell and space group)")

    # Optional assets
    model_manager: Any = Field(default=None, description="MMTBX model manager with geometry restraints")
    restraint_manager: Any = Field(default=None, description="Geometry restraints manager")
    xray_structure: Any = Field(default=None, description="X-ray structure derived from PDB hierarchy")
    macromolecule_metadata: Dict[str, Any] = Field(default=None, description="Macromolecule provenance and quality info")
...
class XrayAtomicModelDataBundle(BaseModel):
    ...
    # Required assets
    xray_structure: Any = Field(description="CCTBX xray.structure object with atomic model")
    miller_indices: Any = Field(description="Miller indices for structure factor calculation")

    # Optional assets
    bulk_solvent_params: Dict[str, Any] = Field(default=None, description="Bulk solvent correction parameters")
...
class ExperimentalDataBundle(BaseModel):
    ...
    miller_indices: Any = Field(description="Miller indices for experimental reflections")

    # Optional assets
    r_free_flags: Any = Field(default=None, description="Free R flags for cross-validation")
    sigmas: Any = Field(default=None, description="Uncertainties in observed structure factors")
    i_obs: Any = Field(default=None, description="Observed intensities (if available)")
    anomalous_data: Dict[str, Any] = Field(default=None, description="Anomalous scattering data (F+, F-, or I+, I-)")
...
class AgentSecurityBundle(BaseModel):
    ...
    # Required assets
    agent_registration: Any = Field(description="Agent registration and validation information")
    permissions: Any = Field(description="List of granted permissions for the agent")

    # Optional assets
    capabilities: Any = Field(default=None, description="Agent capabilities and their schemas")
    whitelisted_modules: Any = Field(default=None, description="Modules the agent is allowed to import and use")
    instruction_triggers: Any = Field(default=None, description="Valid instruction triggers for this agent")
    security_policies: Dict[str, Any] = Field(default=None, description="Security policies for the agent")
...
class RedisStreamsBundle(BaseModel):
    ...
    # Required assets
    stream_configuration: Dict[str, Any] = Field(description="Configuration for Redis streams")
    consumer_groups: Any = Field(description="Consumer group configurations")

    # Optional assets
    message_schemas: Dict[str, Any] = Field(default=None, description="JSON schemas for message validation")
    retry_policies: Dict[str, Any] = Field(default=None, description="Retry policies for failed messages")
    monitoring_config: Dict[str, Any] = Field(default=None, description="Monitoring configuration for streams")
...
class StructureFactorDataBundle(BaseModel):
    ...
    miller_indices: Any = Field(description="Miller indices corresponding to structure factors")

    # Optional assets
    f_mask: Any = Field(default=None, description="Structure factors from bulk solvent mask")
    f_model: Any = Field(default=None, description="Combined structure factors: scale * (f_calc + k_sol * f_mask)")
