...
from ..schemas.generated import AgentSecurityBundle, AgentConfigurationBundle
...
class AgentRegistration(BaseModel):
    ...
    def validate_agent_type(cls, v):
        """Validate agent type."""
        valid_types = ["geometry_agent", "structure_factor_agent", "target_agent", "gradient_agent"]
    ...
    def validate_permissions(cls, v):
        ...
        valid_permissions = {
            "geometry_calculation", "structure_factor_calculation", "target_calculation",
            "gradient_calculation", "bundle_read", "bundle_write", "bundle_delete",
            "coordinate_update", "experimental_data_processing"
        }
        for permission in v:
...
class AgentSecurityManager:
    ...
    def __init__(self, redis_manager: RedisManager):
        ...
        self.default_policy = SecurityPolicy(
            whitelisted_modules=[
                "cctbx", "mmtbx", "iotbx", "libtbx", "scitbx",
                "agentbx.processors", "agentbx.core", "agentbx.schemas",
                "torch", "numpy", "scipy", "asyncio", "json", "time",
                "datetime", "logging", "uuid", "dataclasses"
            ],
            blacklisted_modules=[
                "os", "sys", "subprocess", "multiprocessing",
                "socket", "urllib", "ftplib", "smtplib", "telnetlib"
            ]
        )
    ...
    def check_permission(self, agent_id: str, permission: str) -> bool:
        ...
        if not has_permission:
            self._log_violation(
                ...
                "medium",
                agent_id,
                {"requested_permission": permission, "available_permissions": registration.permissions}
            )
    ...
    def validate_file_access(self, agent_id: str, file_path: str, access_type: str) -> bool:
        """
    ...
    def audit_agent_activity(self, agent_id: str, activity: str, details: Dict[str, Any]) -> None:
        """
    ...
    def _create_security_bundle(self, registration: AgentRegistration) -> AgentSecurityBundle:
    ...
    def _log_violation(self, violation_type: str, message: str, severity: str, 
                      agent_id: str, details: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            violation_key = f"agentbx:violations:{agent_id}:{violation.timestamp.isoformat()}"
            self.redis_manager.cache_set(violation_key, violation.__dict__, ttl=86400)  # 24 hours
        except Exception as e:
            self.logger.error(f"Failed to store violation: {e}")
    
    def _check_suspicious_activity(self, agent_id: str, activity: str, details: Dict[str, Any]) -> None:
