...
class RedisStreamManager:
    ...
    async def _create_dead_letter_stream(self) -> None:
        ...
        try:
            ...
            await self.redis_client.xdel(self.dead_letter_stream, "0-1")  # Remove init message
        ...
        try:
            ...
            if handler.input_schema:
                if not self._validate_message_schema(fields, handler.input_schema):
                    await self._handle_failed_message(message, "Schema validation failed")
                    return
            ...
            if success:
                ...
                await self._handle_failed_message(message, "Processing failed after retries")
                self._update_metrics(False, time.time() - start_time)
            
        except Exception as e:
            self.logger.error(f"Failed to process message {message_id}: {e}")
            await self._handle_failed_message(
                StreamMessage(message_id, fields, datetime.now(), self.stream_name, self.consumer_group),
                str(e)
            )
            self._update_metrics(False, time.time() - start_time)
    
    async def _process_with_retry(self, message: StreamMessage, handler: MessageHandler) -> bool:
        """Process message with retry logic."""
        retry_count = 0
        ...
        while retry_count <= self.retry_policy.max_retries:
            try:
                ...
                if handler.output_schema and not self._validate_output_schema(result, handler.output_schema):
                ...
                self.logger.warning(f"Handler error for message {message.message_id}: {e}")
                retry_count += 1
            
            if retry_count <= self.retry_policy.max_retries:
                # Calculate delay with jitter
                jitter = delay * self.retry_policy.jitter_factor * (2 * (time.time() % 1) - 1)
                actual_delay = min(delay + jitter, self.retry_policy.max_delay_ms / 1000.0)
                
                self.logger.info(f"Retrying message {message.message_id} in {actual_delay:.2f}s (attempt {retry_count})")
    ...
    async def _handle_failed_message(self, message: StreamMessage, error: str) -> None:
        ...
        try:
            ...
            self.logger.warning(f"Message {message.message_id} sent to dead letter queue: {error}")
            
        except Exception as e:
            self.logger.error(f"Failed to handle dead letter message: {e}")
    
    def _validate_message_schema(self, fields: Dict[str, str], schema: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for field_name, field_spec in field_types.items():
                if field_name in fields:
                    ...
                    if expected_type == "string" and not isinstance(fields[field_name], str):
                        return False
    ...
    def _update_metrics(self, success: bool, processing_time: float) -> None:
        ...
        if total_messages > 0:
            self.metrics.average_processing_time = (
                (self.metrics.average_processing_time * (total_messages - 1) + processing_time) / total_messages
            )
    ...
    async def send_message(self, fields: Dict[str, str], handler_name: str = "default") -> str:
        """
            ...
            for message_id, consumer, idle_time, delivery_count in pending:
                ...
                if message_data:
                    ...
                    messages.append(StreamMessage(
                        message_id=message_id,
                        fields=fields,
                        timestamp=datetime.now() - timedelta(milliseconds=idle_time),
                        stream_name=self.stream_name,
                        consumer_group=self.consumer_group
                    ))
