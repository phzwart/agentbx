...
class SolventOptimizer(OptimizationClient):
    ...
    def _load_parameters_from_bundle(self) -> torch.Tensor:
        ...
        try:
            ...
            self.logger.info(f"Loaded solvent parameters: k_sol={param_values[0]:.3f}, b_sol={param_values[1]:.3f}")
            return param_values
    ...
    def _update_parameters_in_bundle(self, new_parameters: torch.Tensor) -> str:
        ...
        try:
            ...
            self.logger.info(f"Updated solvent parameters in bundle: {updated_bundle_id}")
