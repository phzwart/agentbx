...
def analyze_miller_array(miller_array: Any, name: str = "miller_array") -> Dict[str, Any]:
    """
...
def analyze_bundle(bundle: Any) -> Dict[str, Any]:
    ...
    try:
        analysis = {
            ...
            "size_estimate": bundle.get_size_estimate() if hasattr(bundle, 'get_size_estimate') else None
        }
        ...
        for asset_name, asset in bundle.assets.items():
            if hasattr(asset, 'size') and hasattr(asset, 'data'):
                ...
                analysis["assets"][asset_name] = {
                    ...
                    "n_chains": len(asset.chains()) if hasattr(asset, 'chains') else None,
                ...
                analysis["assets"][asset_name] = {
                    ...
                    "str_repr": str(asset)[:100] + "..." if len(str(asset)) > 100 else str(asset)
                }
...
def print_analysis_summary(analysis: Dict[str, Any], indent: int = 0):
    """
    ...
    if "phases" in analysis:
        ...
        logger.info(f"{prefix}  Phase range: {phase['min']:.1f}° to {phase['max']:.1f}°")
...
def compare_structure_factors(f_calc: Any, f_obs: Any, name_prefix: str = "") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        comparison = {
            ...
            "r_factor": r_factor,
            "r_free": r_free,
            "f_calc_analysis": analyze_miller_array(f_calc_common, f"{name_prefix}f_calc"),
