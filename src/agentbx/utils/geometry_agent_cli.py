...
from ..core.async_geometry_agent import AsyncGeometryAgent, GeometryRequest, GeometryResponse
from ..core.agent_security_manager import AgentSecurityManager, AgentRegistration
from ..core.redis_manager import RedisManager
from ..core.redis_stream_manager import RedisStreamManager, MessageHandler, RetryPolicy
...
@click.option('--max-processing-time', default=300, help='Max processing time in seconds')
@click.option('--health-check-interval', default=30, help='Health check interval in seconds')
@click.pass_context
def start_agent(ctx, agent_id, stream_name, consumer_group, max_processing_time, health_check_interval):
...
@cli.command()
@click.option('--agent-id', required=True, help='Agent ID')
@click.option('--macromolecule-bundle-id', required=True, help='Macromolecule bundle ID')
@click.option('--priority', default=1, help='Request priority')
@click.option('--timeout', default=300, help='Request timeout in seconds')
@click.option('--stream-name', default='geometry_requests', help='Redis stream name')
@click.pass_context
def send_request(ctx, agent_id, macromolecule_bundle_id, priority, timeout, stream_name):
    ...
    async def send_request_async():
        ...
        click.echo(f"Sent geometry request {request.request_id} (message ID: {message_id})")
                    ...
                    for msg_id, fields in stream_messages:
                        ...
                        if response.request_id == request.request_id:
                            if response.success:
                                ...
                                click.echo(f"   Bundle ID: {response.geometry_bundle_id}")
    ...
    async def check_status():
        ...
        for key in agent_keys:
            ...
            if agent_data:
                ...
                click.echo(f"Last Heartbeat: {agent_data.get('last_heartbeat', 'Unknown')}")
                ...
                try:
                    ...
                    click.echo(f"Requests Processed: {stats.get('requests_processed', 0)}")
...
@click.option('--permissions', multiple=True, default=['geometry_calculation', 'bundle_read', 'bundle_write'], help='Agent permissions')
@click.pass_context
        ...
        try:
            ...
            for group in groups:
                if group['name'] == consumer_group:
                    ...
                    # Get consumer info
                    consumers = await redis_client.xinfo_consumers(stream_name, consumer_group)
                    click.echo(f"\nConsumers:")
                    for consumer in consumers:
                        click.echo(f"  {consumer['name']}: {consumer['pending']} pending")
