...
class WorkflowManager:
    ...
    def add_workflow_step(self, step_name: str, agent_class, agent_id: str, 
                         input_bundle_types: List[str], output_bundle_types: List[str]):
        """
    ...
    def execute_workflow(self, input_bundle_ids: Dict[str, str]) -> Dict[str, str]:
        ...
        try:
            for i, step in enumerate(self.current_workflow["steps"]):
                logger.info(f"Executing workflow step {i+1}/{len(self.current_workflow['steps'])}: {step['name']}")
                ...
                for bundle_type in step["input_bundle_types"]:
                    if bundle_type in current_bundle_ids:
                        ...
                        raise ValueError(f"Missing input bundle type '{bundle_type}' for step '{step['name']}'")
            ...
            logger.info(f"Workflow completed successfully: {self.current_workflow['id']}")
...
def create_simple_structure_factor_workflow(redis_manager, pdb_file: str, mtz_file: Optional[str] = None):
    """
...
def execute_structure_factor_workflow(redis_manager, pdb_file: str, mtz_file: Optional[str] = None):
    """
    ...
    Returns:
        Dictionary with output bundle IDs
    """
