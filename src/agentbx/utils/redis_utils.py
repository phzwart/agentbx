...
def test_connection(host: str, port: int, db: int, password: Optional[str]):
...
@click.option("--db", default=0, help="Redis database")
@click.option("--password", help="Redis password")
def list_bundles(host: str, port: int, db: int, password: Optional[str]):
...
@click.option("--db", default=0, help="Redis database")
@click.option("--password", help="Redis password")
def show_bundle(bundle_id: str, host: str, port: int, db: int, password: Optional[str]):
    """Show bundle details."""
    try:
        config = RedisConfig(host=host, port=port, db=db, password=password)
        redis_manager = RedisManager(**config.__dict__)
...
def clear_all(host: str, port: int, db: int, password: Optional[str]):
