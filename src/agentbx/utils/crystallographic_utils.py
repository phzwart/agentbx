...
class CrystallographicFileHandler:
    ...
    def read_pdb_file(self, pdb_file: str):
        """
        ...
        try:
            ...
            logger.info(f"Successfully read PDB with {len(xray_structure.scatterers())} atoms")
    ...
    def read_mtz_file(self, mtz_file: str, array_index: Optional[int] = None):
        """
        ...
        try:
            ...
            if array_index is not None:
                if 0 <= array_index < len(miller_arrays):
                    ...
                    logger.info(f"Selected array {array_index}: {selected_array.info().labels}")
                    return selected_array
                else:
                    raise ValueError(f"Array index {array_index} out of range (0-{len(miller_arrays)-1})")
...
def create_atomic_model_bundle(
    ...
    logger.info(f"Created atomic model bundle with {len(xray_structure.scatterers())} atoms")
...
def _create_synthetic_miller_indices(xray_structure, d_min: float = 2.0):
    """
    ...
    try:
        ...
        # Add some metadata
        f_obs.set_info(miller.array_info(
            source="synthetic",
            labels=["F_obs"],
            wavelength=1.0
        ))
        
        logger.info(f"Generated synthetic miller indices: {f_obs.size()} reflections, d_min={d_min}A")
        return f_obs
...
def validate_crystallographic_files(pdb_file: str, mtz_file: Optional[str] = None) -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    try:
        ...
        if pdb_info.get("type") == "error":
            validation_results["errors"].append(f"PDB file error: {pdb_info.get('error')}")
    ...
    try:
        ...
        if mtz_file and os.path.exists(mtz_file):
            ...
            if mtz_info.get("type") == "error":
                validation_results["errors"].append(f"MTZ file error: {mtz_info.get('error')}")
            
            # Check compatibility
            if (validation_results["pdb_file"] and 
                validation_results["mtz_file"] and
                validation_results["pdb_file"].get("type") == "pdb" and
                validation_results["mtz_file"].get("type") == "mtz"):
                
                validation_results["compatibility"] = {
                    "pdb_atoms": validation_results["pdb_file"].get("n_atoms"),
                    "mtz_reflections": validation_results["mtz_file"].get("n_reflections"),
                    "mtz_d_min": validation_results["mtz_file"].get("d_min")
                }
