...
async def demonstrate_coordinate_optimization(redis_manager: RedisManager, macromolecule_bundle_id: str):
...
async def demonstrate_bfactor_optimization(redis_manager: RedisManager, macromolecule_bundle_id: str):
...
async def demonstrate_solvent_optimization(redis_manager: RedisManager, macromolecule_bundle_id: str):
    ...
    try:
        ...
        logger.info(f"grid_resolution_factor: {solvent_stats['grid_resolution_factor']:.3f}")
...
async def demonstrate_sequential_optimization(redis_manager: RedisManager, macromolecule_bundle_id: str):
    ...
    try:
        ...
        coord_results = await demonstrate_coordinate_optimization(redis_manager, macromolecule_bundle_id)
        
        # Step 2: Optimize B-factors
        logger.info("Step 2: Optimizing B-factors...")
        bfactor_results = await demonstrate_bfactor_optimization(redis_manager, macromolecule_bundle_id)
        
        # Step 3: Optimize solvent parameters
        logger.info("Step 3: Optimizing solvent parameters...")
        solvent_results = await demonstrate_solvent_optimization(redis_manager, macromolecule_bundle_id)
        
        # Summary
        logger.info("\n=== Sequential Optimization Summary ===")
        logger.info(f"Coordinate optimization: {coord_results['iterations']} iterations, gradient_norm = {coord_results['final_gradient_norm']:.6f}")
        logger.info(f"B-factor optimization: {bfactor_results['iterations']} iterations, gradient_norm = {bfactor_results['final_gradient_norm']:.6f}")
        logger.info(f"Solvent optimization: {solvent_results['iterations']} iterations, gradient_norm = {solvent_results['final_gradient_norm']:.6f}")
        
        total_time = coord_results['total_time'] + bfactor_results['total_time'] + solvent_results['total_time']
...
async def demonstrate_parameter_inspection(redis_manager: RedisManager, macromolecule_bundle_id: str):
    ...
    try:
        ...
        logger.info(f"Final xray_structure has {len(xray_structure.scatterers())} scatterers")
...
async def main():
    ...
    try:
        ...
        await demonstrate_coordinate_optimization(redis_manager, macromolecule_bundle_id)
        ...
        await demonstrate_sequential_optimization(redis_manager, macromolecule_bundle_id)
