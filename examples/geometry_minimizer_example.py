...
async def run_minimization_example():
    ...
    try:
        ...
        logger.info(
            f"Bundle ID changed: {macromolecule_bundle_id} -> {results['final_bundle_id']}"
        )
        ...
        for i, iteration in enumerate(results["iteration_history"][:5]):  # Show first 5
            logger.info(
                f"Iteration {iteration['iteration']}: gradient_norm = {iteration['gradient_norm']:.6f}"
            )

        if len(results["iteration_history"]) > 5:
            logger.info(
                f"... and {len(results['iteration_history']) - 5} more iterations"
            )
...
async def demonstrate_coordinate_updates():
    ...
    try:
        ...
        if os.path.exists(pdb_file):
            ...
            updated_bundle_id = macromolecule_processor.update_coordinates(
                bundle_id, new_coordinates
            )
