...
class PDBDownloader:
    ...
    def _generate_synthetic_mtz(self, pdb_code: str, output_dir: str) -> str:
        ...
        try:
            ...
            # Write MTZ file
            output_path = os.path.join(output_dir, "input.mtz")
            f_obs.as_mtz_dataset(column_root_label="F_obs").mtz_object().write(output_path)
    ...
    def download_all_data(self, pdb_code: str, output_dir: str = "examples") -> tuple[str, str]:
        """
...
@click.option('--output-dir', '-o', default='examples', help='Output directory for downloaded files')
...
def main(pdb_code, output_dir, list_available, force):
    ...
    if not validate_pdb_code(pdb_code):
        click.echo("Error: PDB code must be 4 alphanumeric characters (e.g., 1ubq, 1ee2, 2pdb)")
        return
