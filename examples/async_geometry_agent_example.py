        ...
        capabilities=[
            {
                ...
                "input_schema": {
                    "type": "object",
                    "properties": {"macromolecule_bundle_id": {"type": "string"}},
                    "required": ["macromolecule_bundle_id"],
                },
                "output_schema": {
                    "type": "object",
                    "properties": {"geometry_bundle_id": {"type": "string"}},
                },
                "timeout_seconds": 300,
                "max_retries": 3,
            }
...
async def start_geometry_agent(
    redis_manager: RedisManager, agent_id: str
) -> AsyncGeometryAgent:
...
async def send_geometry_request(
    ...
    # Read response
    while True:
        messages = await redis_client.xread({response_stream: "0"}, count=1, block=1000)
                ...
                for msg_id, fields in stream_messages:
                    ...
                    if response_data.get("request_id") == request.request_id:
                        if response_data.get("success"):
                            ...
                            print("✅ Geometry calculation completed!")
...
async def demonstrate_coordinate_translation(
    redis_manager: RedisManager, geometry_bundle_id: str
):
    ...
    sample_coordinates = torch.randn(
        100, 3, requires_grad=True
    )  # 100 atoms, 3 coordinates
    ...
    coordinate_bundle_id = translator.register_bundle(
        f"coordinates_{int(time.time())}", tensor_coordinates
    )
    ...
    if tensor_coordinates.requires_grad:
        ...
        translator.torch_to_cctbx(gradients)
        print("   Converted gradients back to CCTBX format")
...
async def demonstrate_stream_management(redis_manager: RedisManager):
    ...
    print(
        f"   Stream metrics: {metrics.messages_processed} processed, {metrics.messages_failed} failed"
    )
...
async def main():
    ...
    try:
        ...
        macromolecule_bundle_id = await create_sample_macromolecule_bundle(
            redis_manager
        )

        # 4. Send geometry request
        print("\n4️⃣ Sending geometry calculation request...")
        geometry_bundle_id = await send_geometry_request(
