...
from agentbx.schemas.generator import (
    SchemaGenerator,
    AssetDefinition,
    ValidationRule,
    WorkflowPattern,
...
def temp_schema_dir():
    ...
    test_schema = {
        ...
        "required_assets": ["test_asset"],
        "optional_assets": ["optional_asset"],
        "asset_definitions": {
            "test_asset": {
        ...
        "dependencies": ["other_schema"],
        "produces_for": ["target_schema"]
    }
    
    schema_file = temp_dir / "test_data.yaml"
...
@pytest.fixture
def complex_schema_dir():
    """Create a temporary directory with complex test schemas."""
    temp_dir = Path(tempfile.mkdtemp())
    
    # Create a complex test schema with list-based validation rules
    complex_schema = {
        "task_type": "complex_data",
        "description": "Complex test schema",
        "required_assets": ["complex_asset"],
        "asset_definitions": {
            "complex_asset": {
    ...
    schema_file = temp_dir / "complex_data.yaml"
    with open(schema_file, 'w') as f:
        yaml.dump(complex_schema, f)
...
class TestAssetDefinition:
    ...
    def test_basic_asset_definition(self):
    ...
    def test_complex_asset_definition(self):
    ...
    def test_asset_definition_defaults(self):
...
class TestWorkflowPattern:
    ...
    def test_basic_workflow_pattern(self):
...
class TestSchemaGenerator:
    ...
    def test_load_schema(self, temp_schema_dir):
        ...
        assert test_asset.checksum_required is True
        ...
        assert optional_asset.default_values == {"value": 42}
        ...
        assert schema.produces_for == ["target_schema"]
    ...
    def test_load_schema_with_list_validation_rules(self, complex_schema_dir):
        ...
        assert rules["complex_data_type"] is True
    ...
    def test_load_schema_with_list_workflow_patterns(self, complex_schema_dir):
        ...
        assert pattern.produces == ["output"]
        assert pattern.validates == ["data_quality"]
    ...
    def test_generate_validators(self, temp_schema_dir):
        ...
        validator_code = "\n".join(validators)
        assert "@validator('test_asset')" in validator_code
    ...
    def test_generate_bundle_model(self, temp_schema_dir):
        ...
        assert 'bundle_type: str = Field(default="test_data", const=True)' in model_code
        assert "created_at: datetime = Field(default_factory=datetime.now)" in model_code
        ...
        # Check validators
        assert "@validator('test_asset')" in model_code
        
        # Check utility methods
        assert "def calculate_checksum(self) -> str:" in model_code
        assert "def validate_dependencies(self, available_bundles: Dict[str, 'BaseModel']) -> bool:" in model_code
    ...
    def test_generate_all_models(self, temp_schema_dir):
        ...
        # Check imports
        assert "from typing import Dict, List, Optional, Any" in all_models
        assert "from pydantic import BaseModel, Field, validator" in all_models
    ...
    def test_write_generated_models(self, temp_schema_dir, tmp_path):
        ...
        assert "from pydantic import BaseModel, Field, validator" in content
...
class TestMainFunction:
    ...
    def test_main_success(self, mock_watch, temp_schema_dir, tmp_path):
        ...
        with patch('sys.argv', ['generator.py', '--schemas-dir', str(temp_schema_dir), '--output', str(output_file)]):
            result = main()
    ...
    def test_main_schema_dir_not_found(self, tmp_path):
        ...
        with patch('sys.argv', ['generator.py', '--schemas-dir', str(non_existent_dir), '--output', str(output_file)]):
            result = main()
    ...
    def test_main_no_schema_files(self, tmp_path):
        ...
        with patch('sys.argv', ['generator.py', '--schemas-dir', str(empty_dir), '--output', str(output_file)]):
            result = main()
    ...
    def test_main_with_watch(self, mock_watch, temp_schema_dir, tmp_path):
        ...
        with patch('sys.argv', ['generator.py', '--schemas-dir', str(temp_schema_dir), '--output', str(output_file), '--watch']):
    ...
    def test_main_with_verbose(self, temp_schema_dir, tmp_path):
        ...
        with patch('sys.argv', ['generator.py', '--schemas-dir', str(temp_schema_dir), '--output', str(output_file), '--verbose']):
            result = main()
...
class TestQuickGenerate:
    ...
    def test_quick_generate_success(self, temp_schema_dir, tmp_path):
        ...
        with patch('agentbx.schemas.generator.Path') as mock_path:
            ...
            mock_path.side_effect = lambda x: temp_schema_dir if "definitions" in str(x) else tmp_path / "generated.py"
...
class TestErrorHandling:
    ...
    def test_asset_definition_with_none_values(self, temp_schema_dir):
        ...
        schema_data = {
            ...
            "required_assets": ["asset1"],
            "asset_definitions": {
                "asset1": None  # This should be handled gracefully
            }
        }
        ...
        generator.load_all_schemas()
        assert len(generator.schemas) == 2  # Should load both schemas (test_data + none_test)
    ...
    def test_workflow_pattern_with_none_values(self, temp_schema_dir):
        ...
        # Should handle None values gracefully
        generator.load_all_schemas()
        assert len(generator.schemas) == 2  # Should load both schemas (test_data + none_pattern_test)
    ...
    def test_validation_rules_with_none_values(self, temp_schema_dir):
        ...
        # Should handle None values gracefully
        generator.load_all_schemas()
        assert len(generator.schemas) == 2  # Should load both schemas (test_data + none_rules_test)
...
def test_main_exception(monkeypatch, temp_schema_dir, tmp_path):
    ...
    monkeypatch.setattr(genmod.SchemaGenerator, "load_all_schemas", lambda self: (_ for _ in ()).throw(Exception("fail!")))
    output_file = tmp_path / "generated.py"
    with patch('sys.argv', ['generator.py', '--schemas-dir', str(temp_schema_dir), '--output', str(output_file)]):
        result = genmod.main()
    assert result == 1 