...
def check_per_file_coverage():
    ...
    for file_path, file_data in data["files"].items():
        ...
        if rel_path in per_file_minimums:
            ...
            if strict_mode and percent_covered < overall_minimum:
                ...
                print(f"❌ FAIL: {rel_path} - {percent_covered:.1f}% < {overall_minimum}% (strict mode)")
            else:
                print(f"ℹ️  INFO: {rel_path} - {percent_covered:.1f}% (no requirement set)")
    ...
    if overall_minimum > 0:
        if total_percent < overall_minimum:
            print(f"❌ Overall coverage {total_percent:.1f}% < required {overall_minimum}%")
            return False
        else:
            print(f"✅ Overall coverage {total_percent:.1f}% >= required {overall_minimum}%")
    ...
    if failed_files:
        ...
        for file_info in failed_files:
            print(f"  - {file_info['file']}: {file_info['coverage']:.1f}% < {file_info['required']}%")
        return False
    else:
